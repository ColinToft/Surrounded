...
public class BallShooter : MonoBehaviour {
    ...
    float ballStartDistance = 0.15f; // space between player and ball
	...
	void Update () {
        if (Input.GetButtonDown("Fire") && !Game.isPaused()) {
            ...
            float adjustedAngle = angle % (90 * Mathf.Deg2Rad); // adjust because we only want a value between 0 and 45 degrees
            if (adjustedAngle > (45 * Mathf.Deg2Rad)) adjustedAngle = (90 * Mathf.Deg2Rad) - adjustedAngle;
            float distance = ballRadius + (playerRadius / Mathf.Cos(adjustedAngle)) + ballStartDistance; // ball radius + formula to calculate distance to edge of square + ballStartDistance (for spacing)
